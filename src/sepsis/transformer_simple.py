...
for filename in tqdm(
    os.listdir("data/challenge-2019/processed_samples/sepsis"), desc="Loading data"
):
...
def estimate_loss():
    ...
    for split in ["train", "val"]:
        ...
        for k in range(eval_iters):
            ...
            losses[k] = F.binary_cross_entropy(
                torch.squeeze(outputs, -1),
                labels,
            ).item()
            accuracies[k] = (
                ((outputs > 0.68) == labels).float().mean().item()
            )
...
for epoch in range(num_epochs):
    ...
    loss = F.binary_cross_entropy(torch.squeeze(outputs, -1), labels)
...
torch.save(model.state_dict(), "transformer_model_100_lr1e3.pth")
