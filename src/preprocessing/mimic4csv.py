...
def read_patients_table(mimic3_path):
    pats = dataframe_from_csv(os.path.join(mimic3_path, 'hosp/patients.csv'))
...
def read_admissions_table(mimic3_path):
    admits = dataframe_from_csv(os.path.join(mimic3_path, 'hosp/admissions.csv'))
...
def read_icustays_table(mimic3_path):
    stays = dataframe_from_csv(os.path.join(mimic3_path, 'icu/icustays.csv'))
...
def read_icd_diagnoses_table(mimic3_path):
    codes = dataframe_from_csv(os.path.join(mimic3_path, 'hosp/d_icd_diagnoses.csv'))
    codes = codes[['icd_code', 'long_title']]
    diagnoses = dataframe_from_csv(os.path.join(mimic3_path, 'hosp/diagnoses_icd.csv'))
...
def read_events_table_by_row(mimic3_path, table):
    nb_rows = {'icu/chartevents': 330712484, 'hosp/labevents': 27854056, 'icu/outputevents': 4349219}
...
def count_icd_codes(diagnoses, output_path=None):
    ...
    codes['stay_count'] = diagnoses.groupby('icd_code')['stay_id'].count()
    codes.stay_count = codes.stay_count.fillna(0).astype(int)
    codes = codes[codes.stay_count > 0]
    if output_path:
        codes.to_csv(output_path, index_label='icd_code')
    return codes.sort_values('stay_count', ascending=False).reset_index()
...
def read_events_table_and_break_up_by_subject(mimic3_path, table, output_path,
    ...
    nb_rows_dict = {'icu/chartevents': 330712484, 'hosp/labevents': 27854056, 'icu/outputevents': 4349219}
