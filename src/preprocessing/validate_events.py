...
recovered = (
    0  # empty stay_ids are recovered according to stays.csv files (given hadm_id)
)
could_not_recover = 0  # empty stay_ids that are not recovered. This should be zero.
icustay_missing_in_stays = (
    0  # stay_id does not appear in stays.csv. We exclude such events.
)

subdirectories = os.listdir(args.subjects_root_path)
subjects: list[str] = list(filter(is_subject_folder, subdirectories))

for subject in tqdm(subjects, desc="Iterating over subjects"):
    try:
        stays_df = pd.read_csv(
            os.path.join(args.subjects_root_path, subject, "stays.csv"), index_col=False
        )
        ...
        print(f"Error during parsing of {subject}: {e}")
